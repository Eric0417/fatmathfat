...
| 清除 S5 集合殘留 | done | S5 全課程改用直線坐標視覺 |
| 年級切換辨識強化 | done | S4 藍色、S5 陶土色與明確標籤 |
| S5 情境挑戰 | done | 每課新增可展開挑戰 |
| 年級切換選中配色 | done | 移除白色選中文字，改用藍／陶土色底與深色文字 |
| 全站品牌中性化 | done | 「數學好好學」；S4/S5 主圖示與 metadata 分流 |
| S5 專屬課程視覺 | done | 八個主題各有專屬視覺或聚焦模式 |
| S5 數學內容修正 | done | 截距式、平行／重合、面積、法線式、λ 定義與步驟補齊 |
| 綜合練習主題覆蓋 | done | S4/S5 各主題至少一題，不再取前 10 題 |
| S5 AI 數學答案驗證 | done | 可解析題型後端重算；錯誤答案或集合欄位整批拒絕 |
