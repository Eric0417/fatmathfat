...
- 定比分點 λ 滑桿改為動態步距並支援自訂上下限、精確輸入與 λ = −1 無定義提示；`cba993d` 已部署。
- 清除 S5 前三課殘留的 Venn 集合視覺，所有 S5 課程改用直線坐標視覺；S4／S5 切換按鈕使用不同顏色與圖示。
- S5 每課新增可展開的「情境挑戰」，答案先隱藏，學生先思考再展開。
- S4／S5 切換按鈕的選中狀態不再使用白色文字；改為 S4 藍底、S5 陶土底搭配深色文字與內框。
- 將全站品牌改為「數學好好學」，S4/S5 使用不同主圖示；PWA、登入頁、郵件與 AI 文案同步更新。
- S5 每課改用專屬視覺：有向線段、定比分點、面積、斜率、直線方程、兩線關係、點線距離與直線系。
...
- 已推送 `5cee239`、深度內容 `e815ca4`、中性數學圖示 `2135b15`、五種直線方程視覺 `be858c8` 與文字邊界留白 `659fc07` 至 GitHub `main`；Render static site 與 API 均已確認上線新版品牌、課程視覺、完整範例、圖示與版面留白。
...
- 任何結構性變更都要重新索引 graph；目前 index 已反映 S5 集合殘留清除與年級切換改版（980 nodes / 2855 edges），且 D-016 至 D-021 已同步 CBM。
...
- λ 滑桿行為見 D-020；預設範圍是 −5 到 5，範圍改變後目前 λ 會自動夾回新範圍內。
- S5 不應再出現 Venn 或集合課程視覺；年級切換的 S4 使用學院藍、S5 使用陶土色，見 D-021。
- 年級切換按鈕的 active 狀態不可使用白色文字；瀏覽器驗證已加入 computed color 檢查。
- S5 內容與品牌修復見 D-022；目前八個 S5 課程視覺由 `S5LessonVisual` 分派，不要在課程頁改回通用 `CoordinateLineLab`。
- S5 AI 生成題目若無法被後端辨識為支援題型會回傳 `502`，前端保留重試；不要放寬為只檢查格式。
