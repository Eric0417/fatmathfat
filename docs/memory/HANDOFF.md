...
- 已部署 `eb7047b` 至 Render；線上寄件來源切換為 `wongeric1417@gmail.com`，`imwong@g.puiching.edu.mo` 驗證碼郵件已確認由 Render egress 寄出，MIME 為單一 `text/plain`。

## 進行中

- 目前無進行中的程式修改；AI 診斷已完成，尚未決定是否實作錯誤分類日誌、重試或 streaming。非學生格式郵箱已部署為 plain text，寄件來源已切換；最後一步是確認 `174549` 是否進入教師收件匣。
