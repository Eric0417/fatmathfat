...
export const s5Lessons: Lesson[] = [
  {
    ...
    examples: [
      ...
      {
        title: '兩點重合的特例',
        statement: '若 P₁ = P₂，則有向數量為 0，兩點距離也是 0。',
        steps: [
          '把 P₁、P₂ 寫成相同坐標。',
          '終點減起點得到 0。',
          '距離公式的平方差均為 0，開根號仍為 0。'
        ],
        conclusion: '重合兩點不構成有方向的位移。',
        explanation:
          '距離非負；數量為 0 與長度為 0 都是重合情況的正確結果。'
      }
    ],
    explanation:
  ...
  },
  {
    ...
    examples: [
      ...
      {
        title: '三等分點與內心',
        statement: '線段 AB 的三等分點可由 λ = 1/2 與 λ = 2 求得；內心坐標則用對應邊長加權。',
        steps: [
          '第一個三等分點取 λ = 1/2，第二個取 λ = 2。',
          '兩點分別代入定比分點公式。',
          '內心公式中，a 是 A 的對邊長，b、c 依序對應 B、C。'
        ],
        conclusion: '三等分點與重心、內心都是定比分點的延伸應用。',
        explanation:
          '只要先確認比例與起終點，同一套線性組合公式可處理多種特殊點。'
      }
    ],
    explanation:
    ...
    detailedNotes: [
      ...
      '三等分點可用 λ = 1/2 與 λ = 2；內心坐標可用各邊對應的對邊長 a、b、c 對頂點加權，與中點和重心公式不同。'
    ],
    keyPoints: [
      'λ > 0 為內分',
      'λ < 0 為外分',
  ...
  },
  {
    ...
    examples: [
      ...
      {
        title: '多邊形分割',
        statement: '五邊形 (4,1)、(3,3)、(1,−2)、(0,4)、(−4,−4) 可依序連成三角形，再求面積相加。',
        steps: [
          '把多邊形頂點畫在坐標平面上，確認環繞順序。',
          '選取不重疊的對角線，把多邊形切成三個三角形。',
          '對每個三角形使用三點面積公式，再相加。'
        ],
        conclusion: '頂點順序改變只影響行列式符號，最後取絕對值不影響面積。',
        explanation:
          '多邊形頂點順序複雜時，先畫圖並選擇不相交的對角線分割，避免重複或漏算。'
      },
      {
        title: '行列式符號與順序',
        statement: '同三個頂點若改以順時針順序代入，行列式只差一個正負號。',
        steps: [
          '保持三個頂點的環繞順序一致。',
          '先算未加絕對值的行列式值。',
          '最後取絕對值的一半，得到相同面積。'
        ],
        conclusion: '面積公式的絕對值消除了頂點方向帶來的符號差異。',
        explanation:
          '判斷共線時不需要絕對值；求面積時則必須保留最後的絕對值。'
      }
    ],
    explanation:
  ...
  },
  {
    ...
    examples: [
      ...
      {
        title: '斜率與傾斜角範圍',
        statement: '若 −1 ≤ k ≤ √3，則傾斜角 α 落在 [0°, 60°] ∪ [135°, 180°)。',
        steps: [
          '把 k 分成 0 ≤ k ≤ √3 與 −1 ≤ k < 0 兩段。',
          '正斜率對應 [0°, 60°]。',
          '負斜率從 135° 趨近 180°，但 180° 不包含。'
        ],
        conclusion: '斜率範圍不是直接對應單一連續的傾斜角範圍。',
        explanation:
          'tan 在 [0°, 180°) 不連續，解範圍時要先依斜率正負分段。'
      }
    ],
    explanation:
    ...
    detailedNotes: [
      ...
      '斜率範圍轉傾斜角範圍時，必須先分段，不能直接把端點 tan 過去。'
    ],
    keyPoints: [
      'k = tan α',
      '垂直線斜率不存在',
  ...
  },
  {
    ...
    examples: [
      ...
      {
        title: '截距與坐標軸面積',
        statement: '若直線的 x 截距為 a、y 截距為 b，則它與坐標軸圍成的三角形面積是 |ab|/2。',
        steps: [
          '三角形頂點為 (0,0)、(a,0)、(0,b)。',
          '底和高分別為 |a| 與 |b|。',
          '面積 = |a| × |b| ÷ 2。'
        ],
        conclusion: '截距式與面積題連用時，正負截距都要先取絕對值。',
        explanation:
          '截距可正可負；求坐標軸圍成圖形的面積時，使用截距的絕對值。'
      }
    ],
    explanation:
    ...
    detailedNotes: [
      ...
      '直線與坐標軸圍成的三角形面積可用 |ab|/2，但要先確認截距不為 0。'
    ],
    keyPoints: [
      '點斜式最常用',
      '斜截式看 y 截距',
  ...
  },
  {
    ...
    examples: [
      ...
      {
        title: '一般式判斷位置',
        statement: 'A₁B₂ − A₂B₁ 與 A₁A₂ + B₁B₂ 可以統一判斷兩線的方向與垂直關係。',
        steps: [
          '先算 A₁B₂ − A₂B₁，判斷兩線是否方向相同。',
          '方向相同時再比較三組係數比例，區分平行與重合。',
          '最後檢查 A₁A₂ + B₁B₂ = 0 是否為垂直。'
        ],
        conclusion: '一般式可處理垂直線，不依賴斜率是否存在。',
        explanation:
          '斜率法直觀但對垂直線有例外；一般式係數條件可一次涵蓋所有位置關係。'
      }
    ],
    explanation:
  ...
  },
  {
    ...
    examples: [
      ...
      {
        title: '點在直線上的特例',
        statement: '若 P(1, 1) 在 x + y − 2 = 0 上，則點到直線的距離為 0。',
        steps: [
          '代入 P 得 1 + 1 − 2。',
          '分子為 0。',
          '距離 = 0 ÷ √2 = 0。'
        ],
        conclusion: '點在直線上時，公式不因分母為正而產生非零結果。',
        explanation:
          '距離公式的零值可用來快速判斷點是否在直線上。'
      }
    ],
    explanation:
  ...
  },
  {
    ...
    examples: [
      ...
      {
        title: '用第二條件定 λ',
        statement: '已知平行直線系 Ax + By + λ = 0 且過點 P，可代入 P 求 λ。',
        steps: [
          '先寫出只保留一個可變參數的直線系。',
          '代入 P 的坐標得到關於 λ 的一次方程。',
          '解出 λ 後代回直線系。'
        ],
        conclusion: '直線系問題的關鍵是先用第一個條件建立家族，再用第二條件定參數。',
        explanation:
          '不同的第二條件只是產生不同的 λ 方程式，解法結構相同。'
      }
    ],
    explanation:
