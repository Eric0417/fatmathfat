...
符合 `7 位數字-1 位數字@g.puiching.edu.mo` 的學校信箱會以學生身分登入；同網域的其他信箱會自動以老師身分登入。外部管理員仍需透過 `ADMIN_EMAILS` 或後台白名單授權。網站需要連線才能登入、同步學習紀錄與使用 AI 老師。Service Worker 只快取靜態資源，登入後若離線會回到登入頁。
